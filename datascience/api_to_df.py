...
def get_query(site_lists, time_delta):
    ...
    :param time_delta: time window, -3 means recent 3 days. Can only be from -1 to -30
    ...
    query_params = {"q": q, "ts": str(int(dt.datetime.timestamp(ts)*1000)), "sort": "published"}
    return query_params

def output_to_df(output,df):
    """
    Convert a single output (100 news) from webhose to pandas data frame and concat to previous data frame
    :param output: a json output from single webhose query
    :param df: previous pandas data frame from previous query, each row represent one news
    :return: pandas data frame, each row represent one news
    """
    output_flat = pd.io.json.json_normalize(output['posts'])
    df_tmp = output_flat[['thread.uuid','author', 'external_links', 'published', 'text','thread.site_full',
                    'thread.site_categories','thread.site_section','thread.section_title',
                    'thread.main_image','thread.social.facebook.comments',
                    'thread.social.facebook.likes','thread.social.facebook.shares','title', 'url']]
...
def api_df(token,site_lists, time_delta, filename):
    ...
    df = output_flat[['thread.uuid','author', 'external_links', 'published', 'text','thread.site_full',
                    'thread.site_categories','thread.site_section','thread.section_title',
                    'thread.main_image','thread.social.facebook.comments',
                    'thread.social.facebook.likes','thread.social.facebook.shares','title', 'url']]
